...
# === 步驟2: 計算 AV, CAV, AAV, CAAV ===
# 僅針對穩定幣分類
stable_df = df[df["classification"] == stablecoin_class]

# BTC 作為市場指數（使用 log_volume 作為市場交易量指數，假設 BTC 不在穩定幣分類中）
...
if btc_df.empty:
    ...
    if av_all:
        ...
        AAV_CAAV_XLSX = f"{OUTPUT_DIR}/AAV_CAAV_results_2025-07-17_stablecoins.xlsx"
        results_df.to_excel(AAV_CAAV_XLSX, index=False)
        print(f"\n穩定幣的 AAV 和 CAAV 結果已儲存至 {AAV_CAAV_XLSX}")

        # 事件日前後 3 天檢查
        check_window = (-3, 3)
        check_df = results_df[(results_df["rel_day"] >= check_window[0]) & (results_df["rel_day"] <= check_window[1])]
        CHECK_WINDOW_XLSX = f"{OUTPUT_DIR}/AAV_CAAV_check_window_2025-07-17_stablecoins.xlsx"
        check_df.to_excel(CHECK_WINDOW_XLSX, index=False)
        print(f"穩定幣的 3 天檢查結果已儲存至 {CHECK_WINDOW_XLSX}")
